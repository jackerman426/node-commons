...
let jobs = new Queue(queueName)

jobs.process(function (job, next) {

  sgMail.send(job['data'], function (error) {
    return next(error)
  })
})

jobs.on('completed', function (job) {
  logger.info('Email Job ' + _.get(job, 'id') + ' succeeded')
})

jobs.on('paused', function () {
  logger.info('The queue has been paused')
})

jobs.on('resumed', function () {
  logger.info('The queue has been resumed')
})

jobs.on('stalled', function (job) {
  logger.info('Job ' + _.get(job, 'id') + ' has been stalled')
})

jobs.on('failed', function (job, result) {
  logger.error('Email Job ' + _.get(job, 'id') + ' failed with error ' + _.get(result, 'message'))
})

jobs.on('ready', function () {
  logger.info('Redis is connected and the queue is ready to accept jobs')
})

jobs.on('error', function (error) {
  logger.error('A queue error happened: ' + error.message)
})
